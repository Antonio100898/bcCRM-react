...
export function UserContextProvider(props: Props) {
  ...
  const updateDepartments = useCallback((summery: ProblemSummery[]) => {
  ...
  const updateShowProblemDialog = useCallback((u: boolean | false) => {
  ...
  const updateCurrentProblem = useCallback((u: Partial<IProblem> | null) => {
    setCurrentProblem(u);
  }, []);
  ...
  const updateRefreshProblemCount = useCallback((u: boolean | false) => {
    setRefreshProblemCount(u);
  }, []);

  const [showLoader, setShowLoader] = useState<boolean>(false);

  const updateShowLoader = useCallback((u: boolean | false) => {
  ...
  const updateAllProblems = useCallback((u: IProblem[]) => {
    setAllProblems(u);
  }, []);

  const [refreshProblems, setRefreshProblems] = useState<boolean>(true);

  const updateRefreshProblems = useCallback((u: boolean) => {
    setRefreshProblems(u);
  }, []);

  const [workers, setWorkers] = useState<IWorker[]>([]);

  const updateWorkers = useCallback((u: IWorker[]) => {
    setWorkers(u);
  }, []);

  const [selectedDepartmentId, setSelectedDepartmentId] = useState<number>(0);

  const updateSelectedDepartmentId = useCallback((u: number | 0) => {
    setSelectedDepartmentId(u);
  }, []);

  const [showScreensMenu, setShowScreensMenu] = useState<boolean>(false);

  const updateShowScreensMenu = useCallback((u: boolean | false) => {
    // console.log("setShowLoader: " + u);
  ...
  const updateProblemTypes = useCallback((u: IProblemType[]) => {
    setProblemTypes(u);
  }, []);

  const login = useCallback(
    async (credentials: LoginCredetials): Promise<User | null> => {
